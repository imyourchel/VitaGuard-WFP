...
<body>
    ...
    <div class="page-header">
        <div class="container-lg">
            <h1><i class="fas fa-stethoscope me-3"></i>Layanan Kesehatan</h1>
            <p class="mb-0 mt-2">Berbagai layanan kesehatan profesional untuk Anda</p>
        </div>
    </div>
        ...
        <div class="container-lg py-4">
            @if($allServices->isEmpty())
                <div class="text-center py-5">
                    ...
                    <h3 class="text-muted">Tidak Ada Layanan</h3>
                </div>
            @else
                            ...
                            <tbody>
                                @forelse($allServices as $key => $service)
                                    <tr>
                                        ...
                                        <td><span class="badge bg-info">{{ $service->category ? $service->category->category_name : '-' }}</span></td>
                                        <td>{{ $service->description }}</td>
                                        <td><span class="badge bg-success">Rp {{ number_format($service->price, 0, ',', '.') }}</span></td>
                                        <td><span class="badge bg-secondary">{{ $service->availability }}</span></td>
                                    </tr>
                                @empty
                                    <tr><td colspan="6" class="text-center py-4 text-muted">Tidak ada layanan</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
